...
def run_backtest(code: str, timeout: int = 60, spread: float = 0.0001) -> dict:
    ...
    try:
        ...
        # Évite l'erreur 502 du template corrompu sur main
        with Sandbox.create() as s:
            ...
            # Exécution de la stratégie d'Adam
            ...
            sharpe = drawdown = total_return = 0.0
            if stdout_text:
            ...
            return {
                ...
                'execution_time_ms': execution_time_ms,
                'stdout': stdout_text,
                'stderr': "".join(execution.logs.stderr) if hasattr(execution.logs, 'logs') and execution.logs and execution.logs.stderr else ""
            }
